...
<head>
    ...
    <script type="text/javascript">
       $(function(){
    		var types=[]; //等级数组（实际用来盛放X轴坐标值）
    		var sales =[]; //数量数组（实际用来盛放Y坐标值）
    	   ...
    	   $.ajax({
    		  url:'${pageContext.request.contextPath }/goods/getTypes.action',
     		  type:'post',
     		  dataType:'json',
     		  success:function(d){
    	   ...
    	   $.ajax({
     		  url:'${pageContext.request.contextPath }/goods/getSales.action',
      		  type:'post',
      		  dataType:'json',
      		  success:function(d){
      			for (var i = 0; i < d.length; i++) {
